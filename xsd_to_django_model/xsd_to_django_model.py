...
def coalesce(name, model):
    for expr, sub in chain(GLOBAL_MODEL_OPTIONS.get('coalesce_fields', {}).items(),
...
class XSDModelBuilder:
    ...
    def make_model(self, typename, ct_def=None, add_fields=None):
        ...
        for attr_name, attr_doc in model.get('add_json_attrs', {}).items():
...
if __name__ == '__main__':
    try:
        ...
        builder.make_models([a.decode('UTF-8') if six.PY2 \
            else a for a in args['<xsd_type>']])
        builder.merge_models()
        builder.write(open(args['-m'], "w"),
                      open(args['-f'], "w"),
                      codecs.open(args['-j'], "w", 'utf-8'))
    except Exception as e:
        e = unicode(e) if six.PY2 else str(e)
